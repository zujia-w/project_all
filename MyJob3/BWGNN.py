    ...
    def forward(self, graph, feat):
        def unnLaplacian(feat, D_invsqrt, graph):
            ...
            for etype in graph.etypes:
                graph.update_all(fn.copy_u('h', 'm'), fn.sum('m', 'h'), etype=etype)
            return feat - graph.ndata.pop('h') * D_invsqrt

        with graph.local_scope():
            # 对所有边类型计算总度数
            total_degrees = torch.zeros(graph.num_nodes()).to(feat.device)
            for etype in graph.etypes:
                total_degrees += graph.in_degrees(etype=etype).float()
            D_invsqrt = torch.pow(total_degrees.clamp(min=1), -0.5).unsqueeze(-1).to(feat.device)
